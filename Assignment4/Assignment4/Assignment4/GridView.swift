...
@IBDesignable class GridView: UIView, GridViewDataSource {
    ...
    func convert(touch: UITouch) -> GridPosition? {
        ...
        let col = touchYPos / gridHeight * CGFloat(self.cols)
        
        let touchXPos = touch.location(in: self).x
        let gridWidth = frame.size.width
        let row = touchXPos / gridWidth * CGFloat(self.rows)
        
        guard touchYPos > 0 && touchYPos < gridHeight
            && touchXPos > 0 && touchXPos < gridWidth
            else {return nil}
